...
git_sync() {
  ...
  if [ "$IS_PULL_REQUEST" != false ]; then
    if [ "$PROJECT_CLONE_URL" != "$PULL_REQUEST_SOURCE_URL" ]; then
      ...
      if [ ! -z "$SHIPPABLE_DEPTH" ]; then
        git_fetch_cmd="git fetch --depth $SHIPPABLE_DEPTH PR"
      fi
      shippable_retry ssh-agent bash -c "ssh-add $SUBSCRIPTION_PRIVATE_KEY_PATH; $git_fetch_cmd"
    fi;
    reset_result=0
    {
    ...
    if [ $reset_result -ne 0 ]; then
      if [ ! -z "$SHIPPABLE_DEPTH" ]; then
        {
          git rev-list HEAD | grep $COMMIT_SHA >> /dev/null 2>&1
        } || {
          echo "The PR was fetched with depth $SHIPPABLE_DEPTH, but the commit $COMMIT_SHA is not present. Please try increasing the depth setting on your project."
        }
      fi
      return $reset_result
    fi
