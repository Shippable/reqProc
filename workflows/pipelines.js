...
function pipelines(externalBag, callback) {
  var bag = {
    rawMessage: externalBag.rawMessage,
    isServiceNode: config.isServiceNode,
  ...
  async.series([
      ...
      _processTaskStep.bind(null, bag),
...
function _getClusterNode(bag, next) {
  if (bag.isSystemNode || bag.isServiceNode) return next();

  var who = bag.who + '|' + _getClusterNode.name;
  logger.verbose(who, 'Inside');

  if (!bag.ciSteps.length) {
...
function _getService(bag, next) {
  if (!bag.isServiceNode) return next();

  var who = bag.who + '|' + _getService.name;
  logger.verbose(who, 'Inside');

  // TODO:
  // use bag.serviceId to call GET on the service so
  // that it can be referenced throughout this workflow.
  // store the result in bag.service
...
function _publishJobNodeInfo(bag, next) {
  ...
  if (bag.ciSteps.length) return next();
...
function _checkInputParams(bag, next) {
  ...
  if (consoleErrors.length > 0) {
    ...
    bag.jobStatusCode = __getStatusCodeByName('failure', bag.isCI);
  } else {
  ...
  bag.builderApiAdapter.getBuildJobById(bag.buildJobId,
    function (err, buildJob) {
      if (err) {
        ...
        logger.warn(msg);
        bag.jobStatusCode =
          __getStatusCodeByName('error', bag.isCI);
      }
      bag.isJobCancelled = false;
      if (buildJob.statusCode ===
        __getStatusCodeByName('cancelled', bag.isCI)) {
  ...
  bag.builderApiAdapter.getJobById(bag.jobId,
    function (err, job) {
      if (err) {
        ...
        bag.consoleAdapter.closeCmd(false);
        bag.isInitializingJobGrpSuccess = false;
        bag.ciJobStatusCode = __getStatusCodeByName('failure', bag.isCI);
      } else {
        bag.ciJob = job;

        if (job.statusCode === __getStatusCodeByName('cancelled', bag.isCI) ||
            job.statusCode === __getStatusCodeByName('timeout', bag.isCI)) {
            bag.isJobCancelled = true;
...
function _validateCIJobMessage(bag, next) {
  ...
  if (consoleErrors.length > 0) {
    ...
    bag.ciJobStatusCode =
      __getStatusCodeByName('failure', bag.isCI);
  } else {
...
function _updateNodeIdInCIJob(bag, next) {
  ...
  var update = {
    node: bag.nodeId,
    statusCode: __getStatusCodeByName('processing', bag.isCI)
  };
...
function _updateService(bag, next) {
  ...
  logger.verbose(who, 'Inside');
  // TODO:
  // call PUT on bag.service to register the buildJobId
  // and any other relevant information. (set to processing)
  return next();
}
...
function _processTaskStep(bag, next) {
  if (bag.isJobCancelled) return next();
  if (bag.jobStatusCode) return next();
  if (bag.isCI) return next();

  var who = bag.who + '|' + _processTaskStep.name;
  logger.verbose(who, 'Inside');

  // initialize isGrpSuccess for TASK step
  ...
  async.eachSeries(bag.inPayload.propertyBag.yml.steps,
    function (step, nextStep) {
      ...
      async.series([
          __generateStepExecScript.bind(null, bag),
          __writeStepExecScript.bind(null, bag),
          __executeManagedTask.bind(null, bag)
        ],
        function (err) {
          if (bag.isGrpSuccess)
            bag.consoleAdapter.closeGrp(true);
...
function __generateStepExecScript(bag, next) {
  ...
  var fileExists = fs.existsSync(strategyPath);
  if (!fileExists) {
    var msg = util.format('No handler found for strategy: %s',
      bag.inPayload.type);
    bag.consoleAdapter.publishMsg(msg);
    bag.isGrpSuccess = false;
    return next(true);
  ...
  var isManagedJob = bag.inPayload.type !== 'runSh' &&
    bag.inPayload.type !== 'runCLI';
...
function __writeStepExecScript(bag, next) {
...
function __executeManagedTask(bag, next) {
...
function _postOutResourceVersions(bag, next) {
  if (bag.isJobCancelled) return next();
  if (!bag.resourceId) return next();
  if (bag.jobStatusCode !== __getStatusCodeByName('success', bag.isCI))
    return next();
...
function _updateJobStatus(bag, next) {
  ...
  if (!bag.jobStatusCode)
    bag.jobStatusCode =
      __getStatusCodeByName('success', bag.isCI);
...
function _cleanIntegrations(bag, next) {
  ...
  var integrationCleanupPath =
    path.resolve(__dirname, '../_common/templates/integrationCleanup.sh');
