...
function inStep(params, callback) {
  var bag = {
    ...
    templatePath: path.resolve(__dirname, 'templates/inStep.ps1'),
    buildInDir: params.rootDir,
    scriptName: 'inStep.ps1',
    builderApiAdapter: params.builderApiAdapter,
    consoleAdapter: params.consoleAdapter
  };

  bag.who = util.format('%s|job|handlers|resources|%s|gitRepo|%s',
    msName, global.config.shippableNodeOperatingSystem, self.name);
...
function _executeScript(bag, next) {
  ...
  if (provider === 'github')
    scriptBag.templatePath =
      path.resolve(__dirname, 'templates/providers/_github.ps1');
  else if (provider === 'bitbucket' || provider === 'bitbucketServer')
    scriptBag.templatePath =
      path.resolve(__dirname, 'templates/providers/_bitbucket.ps1');
