...
export REPOSITORY_URL="<%=propertyBag.normalizedRepo.repositoryUrl%>"
...
git_sync() {
  ...
  if [ "$IS_PULL_REQUEST" != false ]; then
    if [ "$REPOSITORY_URL" != "$PULL_REQUEST_SOURCE_URL" ]; then
      git remote add PR $PULL_REQUEST_SOURCE_URL
      shippable_retry ssh-agent bash -c "ssh-add /tmp/key.pem; git fetch PR"
    fi;
